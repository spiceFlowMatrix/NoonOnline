...
namespace Training24Admin.Controllers
{
    ...
    public class DepositController : ControllerBase
    {
        ...
        [HttpGet("GetSalesAgentDeposit")]
        public IActionResult GetSalesAgentDeposit(int pagenumber, int perpagerecord, string search)
        {
            PaginationModel paginationModel = new PaginationModel
            {
                pagenumber = pagenumber, 
            ...
            try
            {
                ...
                TokenClaims tc = General.GetClaims(Authorization);
                tc.Id = LessonBusiness.getUserId(tc.sub);
                var salesagent = SalesAgentBusiness.GetAgentByUserId(int.Parse(tc.Id));

                List<Deposit> deposits = DepositBusiness.DepositListByAgent(salesagent.Id, paginationModel, out int total);
