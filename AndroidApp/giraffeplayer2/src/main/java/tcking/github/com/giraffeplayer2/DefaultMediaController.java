    ...
    protected void updatePausePlay() {
        ...
        try {
            if (videoView.isCurrentActivePlayer()) {
                ...
                if (playing) {
                    $.id(R.id.app_video_play).image(R.drawable.ic_pause_black);
                } else {
    ...
    protected void show(int timeout) {
        if (!isShowing) {
            if (videoView.getVideoInfo().isShowTopBar() || displayModel == GiraffePlayer.DISPLAY_FULL_WINDOW) {
                $.id(R.id.left_Side_top_box).visible();
                //$.id(R.id.app_video_title).text(videoView.getVideoInfo().getTitle());
            } else {
                $.id(R.id.left_Side_top_box).gone();
            }
            showBottomControl(true);
            isShowing = true;
        }
        $.id(R.id.left_Side_top_box).visible();
       // $.id(R.id.next).visible();
        //$.id(R.id.previous).visible();
        $.id(R.id.app_video_title).text(videoView.getVideoInfo().getTitle());
        $.id(R.id.app_video_subtitle).text(videoView.getVideoInfo().getSubTitle());
        $.id(R.id.app_video_username).text(videoView.getVideoInfo().getuserName());
...
//        $.id(R.id.app_video_fullscreen).visibility(show ? View.VISIBLE : View.GONE);
        $.id(R.id.app_video_loadingLay).gone();
    ...
    protected void hide(boolean force) {
        if (force || isShowing) {
            ...
            $.id(R.id.next).gone();
            $.id(R.id.previous).gone();
            $.id(R.id.left_Side_top_box).gone(); /*$.id(R.id.app_video_top_box).gone();*/
            $.id(R.id.app_video_loadingLay).gone();
//            $.id(R.id.app_video_fullscreen).invisible();
    ...
    public DefaultMediaController(Context context,Onbackclick backclick){
        super(context);
            ...
            try {
                ...
                if (v.getId() == R.id.app_video_fullscreen) {
                    ...
                    if (displayModel == GiraffePlayer.DISPLAY_FULL_WINDOW) {
                        ...
                        $.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen);
                        $.id(R.id.app_video_finish).image(R.drawable.ic_arrow_back_black_24dp);

                    } else if (displayModel == GiraffePlayer.DISPLAY_NORMAL) {

                        ((Activity) context).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                        $.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_exit);
                        $.id(R.id.app_video_finish).image(R.drawable.ic_keyboard_arrow_down);

                    }
                    player.toggleFullScreen();
                    backclick.fullscreenperfome();
...
//                        Toaster.show("download rate:" + extra);
    ...
    public void onCurrentStateChange(int oldState, int newState) {
        if (context instanceof Activity) {
            if (newState == GiraffePlayer.STATE_LAZYLOADING) {
                $.id(R.id.app_video_loadingLay).gone();
               /* $.id(R.id.app_video_status).visible().id(R.id.app_video_status_text)
    ...
    private void statusChange(int status) {
        ...
        switch (status) {
            case STATUS_LOADING:
                $.id(R.id.app_video_loadingLay).visible();
    ...
    public void onDisplayModelChange(int oldModel, int newModel) {
        ...
        Log.e("CONST", "=====displayModel====" + displayModel);
...
//        }
    ...
    public void onLazyLoadProgress(GiraffePlayer giraffePlayer, int progress) {
        ...
        //$.id(R.id.app_video_status_text).text(context.getString(R.string.giraffe_player_lazy_loading, progress));
    }
    ...
    public void onLazyLoadError(GiraffePlayer giraffePlayer, String message) {
        $.id(R.id.app_video_loadingLay).visible();
        //$.id(R.id.app_video_status).visible();
        //$.id(R.id.app_video_status_text).text(context.getString(R.string.giraffe_player_lazy_loading_error, message));
    }
