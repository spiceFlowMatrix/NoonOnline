...
public class AssignmentListAdapter extends RecyclerView.Adapter<AssignmentListAdapter.MyViewHolder> implements DroidListener {

    List<AssignmentObject.Data> list;
    Context ctx;
    ...
    public AssignmentListAdapter(Context ctx, List<AssignmentObject.Data> list, UserDetails userDetailsObject) {
        this.list = list;
        this.ctx = ctx;
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        AssignmentObject.Data model = list.get(position);
        if (!TextUtils.isEmpty(model.getName())) {
            holder.assignmentItemLayoutBinding.txtAssigignmentName.setText(model.getName());
        }

        if (!TextUtils.isEmpty(model.getSubmissioncount())) {
            holder.assignmentItemLayoutBinding.txtSubmitionCount.setText(model.getSubmissioncount() + " " + ctx.getString(R.string.submition));
        }

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent i = new Intent(ctx, AssignmentDetailActivity.class);
                i.putExtra(Const.AssignmentId, model.getId());
                ctx.startActivity(i);
            }
        });
    }
